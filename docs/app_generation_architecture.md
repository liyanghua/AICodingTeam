...
| CodeAgentExecutor | 代码修改执行层 | 读取完整应用上下文、生成 patch、运行验证、返回 diff | 自由对话、绕过用户确认、直接写事实源 |

工作台负责事实、边界和可观测；PI Agent 负责推理、对话和工具执行；PiAgentProvider 只负责把两者安全、可审计地接起来。

代码修改权威必须唯一。右侧 PI-Agent 不应和中间 Codex/Code Agent 各自形成一套代码修改链路。所有真正改代码的能力都必须收敛到 `CodeAgentExecutor` 抽象下；Codex、PI-code 或通用 LLM-code 只能作为该抽象的不同 Provider。

## V2 生成画布架构定位

V2 生成画布规范见 [`docs/app_generation_canvas_experience_spec.md`](app_generation_canvas_experience_spec.md)，技术方案见 [`docs/app_generation_canvas_technical_plan.md`](app_generation_canvas_technical_plan.md)。

V2 生成画布是 Dashboard 的 **业务对象投影层**，不是新的编排 runtime。架构关系如下：

```text
Team Runtime / app_generation artifacts
-> NodeContext
-> CanvasProjection
-> CanvasObject / ContextObject / BusinessNode
-> CanvasSelectionContext
-> AgentBridge
-> Existing controlled actions
```

架构约束：

- `CanvasProjection` 必须能从 run artifacts、preview record、evaluation artifacts 和 adjustment events 重建。
- `CanvasObject` 和 `ContextObject` 只保存摘要、状态、引用和证据，不保存 secret、完整源码、完整 stdout 或完整 prompt。
- V2 业务节点只是 V1 runtime 节点的业务语言映射，不改变底层执行顺序。
- 右侧 Agent 在 V2 中围绕 `CanvasSelectionContext` 协作，但修改事实源仍必须进入 V1 受控 API。
- 已发布应用修复仍走 `patch_app` 或 `delegate_code_repair`；复杂代码修改仍由 `CodeAgentExecutor` 执行。
- 业务对象编辑不直接覆盖旧 artifact，只能形成 user override、object patch 候选或新 run 输入。

V2 画布第一阶段应以只读投影和对象选择为主，避免一次性重写现有工作台。完整画布化、ContextObject 登记和版本回放应分阶段实现。

短期内，右侧 Agent 可通过 `patch_app` 做小范围、锚点明确的已发布应用修复。长期内，复杂修复必须走 `delegate_code_repair`：PI-Agent 理解用户反馈并生成 `repair_request`，Code Agent 读取完整上下文后生成 patch/diff。
...
-> 用户确认
-> Dashboard 调用 read_artifact / patch_app / delegate_code_repair / rerun_from_node / override / select_variant 等受控 API
-> 新状态回到中间节点事实层
```
...
- 修改已发布应用的小范围问题可走 `patch_app`，但目标必须是 `generated_apps/<slug>/<file>`，且必须写 `app_patches/` 证据。
- 复杂代码修复走 `delegate_code_repair`，由 Code Agent 生成 patch；右侧 PI-Agent 不直接写代码。
