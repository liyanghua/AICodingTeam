...
- 将六个业务节点作为默认用户视图，同时保留 V1 runtime node 事实源。
...
目标：把现有 run artifacts 转成业务用户能理解的六个节点和 `CanvasObject` 列表，先不做自由拖拽画布。
...
2. 固定六个业务节点，并维护 V1 runtime node 到 V2 业务节点的映射。
...
### V2.1：对象化画布与 Agent 动作闭环

目标：把中间区升级为“业务节点轨道 + 对象画布 + 对象详情”，并让右侧 Agent 围绕选中对象生成可确认动作。
...
1. 将中间主视图从节点详情升级为对象画布，保留开发者详情入口。
2. 按对象类型、状态和业务节点展示能力、页面、数据、provider、预览、缺口和修复候选。
3. `intent=auto` 优先读取选中对象和用户消息，再回退 V1 node context。
4. 将 `repair_generated_app` 映射到 `patch_app` 或 `delegate_code_repair`。
5. 将 `verify_capability` 映射到 preview health、runtime smoke 或后续 capability scanner。
6. 对修改事实源的动作展示确认卡、diff、验证计划和回滚入口。
...
- `CanvasProjectionBuilder` 能从 fixture run 生成六个业务节点。
...
5. V2.1 画布：对象画布主视图、状态筛选、证据预览和版本事件入口。
