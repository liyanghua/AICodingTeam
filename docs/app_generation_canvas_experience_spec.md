...
V2 不删除节点流，而是把节点流降级为“业务过程轨道”。用户默认看到业务节点和对象，技术节点、raw artifact、executor、provider 和 JSON 证据只在展开详情中出现。

## 页面结构

### 总体布局

V2 默认仍保留三大区，但中间区从“节点列表 + 节点详情”升级为“生成画布”：

```text
┌──────────────────┬─────────────────────────────────────────────┬──────────────────┐
│ 任务 / 版本列表   │ 生成画布                                    │ 右侧 Agent        │
│                  │ ┌──────────────┬──────────────────────────┐ │                  │
│ runs             │ │ 业务节点轨道   │ 对象画布 / 对象详情       │ │ 对话 / 动作确认    │
│ versions         │ │ 状态时间线     │ 预览 / 证据 / diff        │ │ 修复 / 解释 / 对比  │
└──────────────────┴─────────────────────────────────────────────┴──────────────────┘
```

中间生成画布包含三个区域：

- **业务节点轨道**：展示从 PRD 到交付的业务过程。
- **对象画布**：展示当前 run 的业务对象、依赖关系、状态和能力缺口。
- **对象详情区**：展示当前选中对象的摘要、输入、输出、证据、可编辑项和 Agent 可执行动作。
...
V2 默认只展示业务友好节点标题。内部 node id 仍保留，用于映射现有 runtime artifacts。
